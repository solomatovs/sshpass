...
impl PluginLoader {
    ...
    pub fn reload_plugins(config_path: &str, ctx: &mut UnixContext) -> Result<Vec<ManagedPlugin>, String> {
        ...
        let plugin_section = config.get("plugins")
            .ok_or_else(|| "Некорректный формат config.toml: отсутствует секция plugins".to_string())?;
        ...
        for plugin_value in plugin_order {
            ...
            let (plugin_name, required) = match plugin_value {
                ...
                Value::Table(table) => {
                    let name = table.get("name")
                        .and_then(|n| n.as_str())
                        .ok_or_else(|| "Имя плагина должно быть указано в поле 'name'".to_string())?;
                    
                    let required = table.get("required")
                        .and_then(|r| r.as_bool())
                        .unwrap_or(true); // По умолчанию обязательный
                    
                    (name, required)
                },
                _ => return Err("Элемент массива plugins.order должен быть строкой или таблицей".to_string())
            };
            ...
            match ManagedPlugin::new(plugin_name, ctx) {
                ...
                Err(e) => {
                    if required {
                        return Err(format!("Не удалось загрузить обязательный плагин {}: {}", plugin_name, e));
                    } else {
                        load_errors.push(format!("Пропуск необязательного плагина {}: {}", plugin_name, e));
                    }
                }
            }
        }
